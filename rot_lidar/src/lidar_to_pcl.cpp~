...
//	lidar mx28 axis perpendicular modeg
	...
	tf::Transform m2(q2);
	tf::Transform m3;	
	m3 = m2*m1; // rotate lidar axis and revolute mx28 axis
	rot_tf.transform.rotation.x = m3.getRotation().x();
	rot_tf.transform.rotation.y = m3.getRotation().y();
	rot_tf.transform.rotation.z = m3.getRotation().z();
	rot_tf.transform.rotation.w = m3.getRotation().w();
