...
class Slic {
    ...
    public:
        ...
        int  get_center_num(IplImage *image);
        CvPoint get_center_grid(IplImage* image,int idx);
};
...
int Slic::get_center_num(IplImage *image){
  return (int)centers.size();
}
CvPoint Slic::get_center_grid(IplImage* image,int idx){
  CvPoint center;
  center = cvPoint(centers[idx][3],centers[idx][4]);
  return center;
}

/*std::vector<double> Slic::get_center_grid(IplImage *image){
    std::vector<double> v;
    for (int i=0; i<(int)centers.size();i++){
         cvPoint cen = cvPoint(centers[i][3], centers[i][4]);
         cen.x
    }
}*/
/*
